...
| ⚡ **SSE 流式输出** | Server-Sent Events 实时打字机效果，推理面板自动展开、答案逐字呈现，流式中可随时停止（按钮变红中断，已生成内容保留）|
| 🧠 **智能降级回答** | 检索低相关/无命中时降级为「文档背景 + 常识」回答，泛化问题（如「怎么改进简历」）不再硬拒绝 |
| 📎 **文档引用说明** | 上传文档时 LLM 自动生成引用说明（2-3 句话），拼接到每个 chunk 前面，解决 meta-question（"我的简历怎么样"）检索盲区 |
| 🌐 **联网搜索兜底** | 知识库无答案时调 Tavily 联网搜索补充外部信息；显式命令（"联网搜索"）直接触发；用户可控开关 |
...
检索 → 评分（rerank top score）
  → score≥0.5 → RAG 生成（文档有直接答案直接回答，有背景给分析建议）
...
# SUMMARIZE_ROUND_THRESHOLD=8   # 累计轮数达此值触发会话摘要
...
主要覆盖（125 用例）：文档分块、Embedding 维度、认证、缓存（三防 + 多轮版本隔离 + 历史缓存）、限流、会话记忆（token 截断/摘要注入/摘要生成）、query 改写、CRAG 循环重查（变体生成/评分节点/检索优先级）、文档增量更新（集合 diff/降级全量/旧数据兼容）、中英混排等。
